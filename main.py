...
class QuestionNotCompletedError(RuntimeError):
    """答题器未执行有效点击或输入。"""
...
def solve_english_to_chinese(device, db, relax_time):
        ...
        for result in results:
            ...
            for btn, pfx in zip(buttons, prefixes):
                ...
                if choice_text in clean or clean in choice_text:
                    return _click_choice(btn, choice_text, "ok", choice_text, word, relax_time)
            ...
            for btn, pfx in zip(buttons, prefixes):
                ...
                if choice_cn in clean_cn or clean_cn in choice_cn:
                    return _click_choice(btn, btn.text.replace(pfx, ""), "ok", btn.text.replace(pfx, ""), f"{word} · 汉字兜底", relax_time)
    ...
    if max(rates) >= 0.5:
        ...
        return _click_choice(buttons[idx], choices_text[idx], "info", choices_text[idx], f"{word} · 相似度匹配", relax_time)
    ...
    for part in parts:
        for elem in clickable:
            if elem.text == part:
                ...
                break
    time.sleep(relax_time)
    return bool(parts) and clicked_parts == len(parts)
...
def solve_mixed(device, db, relax_time):
        ...
        for result in results:
            ...
            for i, btn in enumerate(buttons):
                ...
                if clean_result == choice_text:
                    return _click_choice(btn, clean_result, "ok", clean_result, "题库直出", relax_time)
    ...
    if _is_chinese(text):
        ...
        if max(rates) >= 0.5:
            idx = rates.index(max(rates))
            return _click_choice(buttons[idx], choices_text[idx], "info", choices_text[idx], "语义相似度匹配", relax_time)
...
def solve_listening(device, db, relax_time):
    ...
    if answer:
        ...
        for btn, valid in candidates:
            if answer in valid:
                return _click_choice(btn, answer, "ok", answer, "题库直出", relax_time)
...
def solve_question(qtype, device, db, relax_time):
    completed = SOLVERS[qtype](device, db, relax_time)
    if not completed:
        raise QuestionNotCompletedError("答题器未执行任何有效操作")
        ...
        while True:
            ...
            for i in range(total):
                ...
                try:
                    ...
                    solve_question(qtype, device, db, relax_time)
